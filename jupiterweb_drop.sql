drop table if exists rel_cur_tri cascade;

drop table if exists rel_dis_mod cascade;

drop table if exists pf_se cascade;

drop table if exists us_pf cascade;

drop table if exists usuario cascade;

drop table if exists "Planeja" cascade;

drop table if exists "Administra" cascade;

drop table if exists "Administrador" cascade;

drop table if exists periodo_curriculo cascade;

drop table if exists disciplinas_rel_cur_tri cascade;

drop table if exists "Trilha" cascade;

drop table if exists permissoes_perfil cascade;

drop table if exists "Perfil" cascade;

drop table if exists servico cascade;

drop table if exists grade_curriculo cascade;

drop table if exists curriculo cascade;

drop table if exists periodo_disciplina cascade;

drop table if exists prereq_rel_dis_mod cascade;

drop table if exists modulo cascade;

drop table if exists "Cursa" cascade;

drop table if exists "Aluno" cascade;

drop table if exists "Ministra" cascade;

drop table if exists "Disciplina" cascade;

drop table if exists "Professor" cascade;

drop table if exists "Pessoa" cascade;

drop function if exists cpf_validar(varchar);

drop type if exists endereco;

drop type if exists id_ministra;

